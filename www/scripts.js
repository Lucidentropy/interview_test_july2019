...
$(function () {
    ...
    var fetch = function fetch() {
        return $.get('/sql.php', function (data) {
            ...
            if (data.length) {
                data.forEach(function (row) {
                    ...
                    if (row.sale_amounts && row.sale_dates) {
                        ...
                        var amounts = row.sale_amounts.split(',');
                        var dates = row.sale_dates.split(',');
                        var td = $('<td class="sales-history">');

                        amounts.forEach(function (sale, index) {
                            var price = parseInt(sale).toLocaleString();
                            var dateTime = new Date(Date.parse(dates[index]));
                ...
                var nodata = '<tr><td colspan="3">No properties to list.</td></tr>';
    ...
    $('#submitHomeAdd').click(function () {

        var cereal = new Array($('#updateAddress').val(), $('#updateCity').val(), $('#updateState').val(), $('#updateZip').val()).join('|');
        $.ajax({
            type: "POST",
            url: '/sql.php',
            data: {
    ...
    $('#submitHomeEdit').click(function () {
        var id = $('#submitHomeEdit').attr('data-id');
        var cereal = new Array(id, $('#updateAddress').val(), $('#updateCity').val(), $('#updateState').val(), $('#updateZip').val()).join('|');
    ...
    $('#submitSale').click(function () {
        var id = $('#submitSale').attr('data-id');
        var timestamp = new Date($('#salesDate').val()).toJSON().slice(0, 19).replace('T', ' ');
        var cereal = new Array(id, timestamp, $('#salesAmount').val()).join('|');
    ...
    $("#manage button.new").click(function () {
        $('#propertyModal input.form-control').val('');
        $("#submitHomeAdd").show();
        $("#submitHomeEdit").hide();
    ...
    $("#table-all").on('click', '[data-id] button.edit', function (e) {
        var id = $(e.target).closest('td').attr('data-id');
        var data = JSON.parse($(e.currentTarget).attr('data-raw'));
